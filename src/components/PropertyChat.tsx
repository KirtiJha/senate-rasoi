...
/**
 * Collapsible Q&A thread on a property — neighbours ask the owner for details
 * (floor, availability, etc.) without leaking phone numbers; the society can
 * read the answers. Realtime-backed.
 */
export function PropertyChat({ propertyId, ownerUserId, ownerName, accent }: Props) {
  ...
  const load = useCallback(async () => {
    try { setMessages(await fetchPropertyMessages(propertyId)); }
    catch { /* best-effort */ }
  ...
  useEffect(() => {
    ...
    load();
    const unsub = subscribeToPropertyMessages(propertyId, load);
    return unsub;
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      const msg = await sendPropertyMessage(propertyId, userId, text);
    ...
    <View className="mb-4 overflow-hidden rounded-2xl border border-line bg-surface">
      <Pressable onPress={toggle} className="flex-row items-center gap-3 p-4 active:bg-inset">
        <View className="h-9 w-9 items-center justify-center rounded-full" style={{ backgroundColor: accent + '20' }}>
          <Ionicons name="help-circle-outline" size={19} color={accent} />
        </View>
