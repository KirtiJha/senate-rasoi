...
export function Wordmark({ size = 20, markOnly = false }: { size?: number; markOnly?: boolean }) {
  ...
  return (
    <View className="flex-row items-center gap-2">
      ...
      >
        <MaterialCommunityIcons name="pot-steam" size={size * 0.82} color={c.onAccent} />
      </LinearGradient>
      {markOnly ? null : (
        <Text style={{ fontSize: size }} className="font-display text-ink">
          Senate <Text className="text-accent">Rasoi</Text>
        </Text>
      )}
    </View>
  );
...
export function Brandfull() {
  ...
  return (
    <View className="items-center">
      ...
      >
        <MaterialCommunityIcons name="pot-steam" size={38} color={c.onAccent} />
      </LinearGradient>
