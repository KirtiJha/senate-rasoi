...
export default function SportGroupScreen() {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    try {
      const [g, m, t] = await Promise.all([fetchGroup(id), fetchGroupMembers(id), fetchTournaments(id)]);
  ...
  const toggleJoin = async () => {
    ...
    try {
      if (isMember) await leaveGroup(group.id, userId);
      else await joinGroup(group.id, userId);
      await load();
  ...
  const onRemoveMember = (m: GroupMember) => {
    if (!group) return;
    const run = async () => { try { await removeMember(group.id, m.user_id); await load(); } catch { toast.show('Could not remove'); } };
  ...
  const onDeleteGroup = () => {
    if (!group) return;
    const run = async () => { try { await deleteGroup(group.id); toast.show('Group deleted'); router.back(); } catch { toast.show('Could not delete'); } };
  ...
  if (!group) {
    return (
      <View className="flex-1 bg-bg">
        <ScreenHeader title="Group" showBack />
        <View className="flex-1 items-center justify-center px-8"><Text className="text-center text-muted">This group no longer exists.</Text></View>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-bg">
                  ...
                  <View key={t.id} className="flex-row items-start gap-3 rounded-xl bg-inset p-3">
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-[12px] text-muted">
                        {[t.event_date, t.location].filter(Boolean).join(' · ') || 'Date TBA'}
                      </Text>
                      {t.notes ? <Text className="mt-0.5 text-[12px] text-faint">{t.notes}</Text> : null}
                    </View>
                    {canManage ? (
                      <Pressable onPress={async () => { try { await deleteTournament(t.id); await load(); } catch { toast.show('Could not delete'); } }} hitSlop={6}>
      ...
      <AddMemberSheet
        ...
        c={c}
        onAdd={async (uid) => {
          try { await addMember(group.id, uid); setShowAddMember(false); await load(); }
      ...
      <AddTournamentSheet
        ...
        c={c}
        onAdd={async (form) => {
          try { await addTournament({ groupId: group.id, ...form }); setShowAddTourney(false); await load(); }
          catch { toast.show('Could not add'); }
        }}
      />
    </View>
