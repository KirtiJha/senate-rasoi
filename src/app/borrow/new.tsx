...
export default function NewLendItemScreen() {
  ...
  const submit = async () => {
    if (!userId) return;
    if (isOffer && photoFlagged) return toast.show("This photo doesn't match the item — change or remove it ⚠️");
    if (!title.trim()) return toast.show(isOffer ? 'What are you lending?' : 'What do you need to borrow?');
    setSubmitting(true);
    try {
      const item = await postItem({
        communityId, ownerUserId: userId, kind, title, description: desc || null,
        category, photoUri: isOffer ? photo : null, contactWhatsapp: wa || null, contactPhone: null,
      });
      toast.show(isOffer ? 'Listed to lend 🤝' : 'Request posted 🙏');
        ...
        <Container narrow>
          ...
          <View className="mb-5 flex-row rounded-2xl p-1" style={{ backgroundColor: c.inset }}>
            {([['offer', '🤝 I\'m lending this'], ['request', '🙏 I need to borrow']] as [LendKind, string][]).map(([k, lbl]) => (
              ...
              >
                <Text className="text-[12px] font-sans-sb" style={{ color: kind === k ? ACCENT : c.muted }} numberOfLines={1}>{lbl}</Text>
              </Pressable>
            ))}
          </View>
          ...
          <Text className={label}>{isOffer ? 'What are you lending?' : 'What do you need to borrow?'}</Text>
          <TextInput
            value={title} onChangeText={setTitle}
            placeholder={isOffer ? 'e.g. Bosch drill machine' : 'e.g. Ladder, folding chairs…'}
          ...
          <Text className={label}>{isOffer ? 'Notes (optional)' : 'Details (optional)'}</Text>
          <TextInput
            value={desc} onChangeText={setDesc}
            placeholder={isOffer ? 'Condition, how long it can be borrowed, etc.' : 'For how long, any specific requirements, etc.'}
            placeholderTextColor={c.faint}
            multiline
          ...
          <Button
            label={isOffer ? 'List it to lend' : 'Post my request'}
            icon="checkmark"
            size="lg"
            fullWidth
            loading={submitting}
