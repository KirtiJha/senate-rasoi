...
| Date | What changed |
|------|-------------|
| 2026-06-10 | **Teal rebrand (branch `rebrand-teal-logo`, preview — NOT on main).** New brand identity from `assets/images/aangan_*` (teal courtyard-arch logo). Full re-theme: accent **coral `#FF5A3C` → teal `#0F6E56`** (theme.ts + global.css, light+dark); **orange `#E8650A`** as the secondary highlight (food category colour, formerly coral). New `BrandMark` (RN-SVG port of `aangan_icon.svg`); `Brandfull` lockup gains the "your society · your courtyard" tagline. Regenerated icon/favicon/splash/adaptive from the SVG (`gen-icon.mjs`); splash + adaptive + PWA manifest/theme-color → teal. Landing recoloured (teal CTAs/gradients, orange food). **Logo iterated to the multicolour "diversity flower"** (`aangan_diversity_icon_dark.svg`): `BrandMark` = dark tile + 8 colour petals + orange centre; app icon/favicon/splash regenerated from it (splash/adaptive/PWA bg → `#1A1A1A`); landing footer lockup = the flower emblem + "aangan" + colour-dash divider + "every home. every language. one courtyard." tagline. Teal theme/accent kept. Awaiting visual review before merging to main. |
