...
| Date | What changed |
|------|-------------|
| 2026-06-09 | **Notifications + listings polish.** Notifications now include the user's **own** actions (dropped the `actor_id != me` filter — verified own announcement appears). Modal gained per-row **read/unread toggle** + **Mark all read/unread** (added `markUnread`, verified read-row delete under RLS). **Bell moved** off the mobile TopBar to the Home greeting row (far right, next to "Hi, <name>"), mobile-only; desktop keeps the NavRail bell. **All Listings** redesigned from a card grid into a responsive **table** — desktop columns (Listing · Category · Price · View/Contact actions), compact rows + WhatsApp quick-contact on mobile. tsc clean. |
